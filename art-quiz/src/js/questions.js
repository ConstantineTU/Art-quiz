...
class Questions {
	...
	async loadAuthorPictures(number, numberJ) {
		...
		if (location) {
			...
			routing.locationResolver(location, [imgFull.src, imgMini.src, pictures, author, authors, imageNum, name, year, this.j])
			this.j++
	...
	async getRandomAuthors(data, author) {
		...
		for (let i = 0; i < 3; i++) {
			await new Promise(async (resolve, reject) => {
				let randomNum = this.getRandomNumber(0, data.length - 1)
				let randomAuthor = await data[randomNum].author
				let repeat = 0
				for (let j = 0; j < 3; j++) {
